...
① Claude → request.md 초안 작성
   │  - 기능 설명 / API / 비즈니스 규칙
   │  - "확정 설계 결정사항" 섹션에 Y/N 질문 나열
   │    (CLAUDE.md 규칙에서 Claude 가 뽑아냄: 인증 / 큐 / 트랜잭션 등)
   ▼
② 사용자 → 답변 채우기 + 비즈니스 규칙 보완
   │
   ▼
③ Claude → work.md 작성
   │  - 파일 목록 / DTO / Repository / Service / Controller / 테스트 케이스
   │  - CLAUDE.md 규칙 중 src/ 에 없는 인프라가 있으면
   │    "사전 구현 필요 항목" 섹션에 체크리스트로 명시
```
...
당신이 큐 인프라를 먼저 구현해 `[x]` 로 체크한 후에만 `"작업 시작"` 이 진입 가능. **본 기능 구현과 인프라 구현이 섞이는 것을 방지**합니다.
...
| 명령 | 전제 |
|---|---|
| `작업 시작` | work.md 존재 + request 결정사항 전부 답변 + 사전 구현 항목 전부 `[x]` + validator 통과 |
