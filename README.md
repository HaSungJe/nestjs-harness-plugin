...
| **기획** | `<featureName> 기능 생성` | `/feature-plan <featureName>` | 신규 기획 — request → work 작성 |
|  | `<featureName> 기능 수정` | `/feature-modify-plan <featureName>` | 변경 기획 — change.md 작성 |
| **구현** | `작업 시작` 또는 `<featureName> 작업 시작` | `/feature-implement` 또는 `/feature-implement <featureName>` | 신규/수정 자동 라우팅 — 구현 + 테스트 + 자가수복 + 리포트. **인자 없이** 호출하면 미구현 플랜 자동 탐색·1건 선택, **인자 주면** 해당 기능에 한정해 모드 결정 |
...
기획은 **신규(`/feature-plan`)** 와 **변경(`/feature-modify-plan`)** 두 줄기로 갈리지만, **구현은 `/feature-implement` 한 명령으로 통합** — work.md / change.md 중 무엇이 미구현 상태인지 자동 판별해서 진입합니다.
...
|---|---|---|---|
| ① | **사람** | 기능을 구두로 설명 | — |
| ② | Claude | **브랜치 분기 결정** → 사전 검사 (이미 자동 브랜치 위면 질의 생략·이어가기) / 신규 사이클이면 "새 작업 브랜치 만들까요?" 질의 → 예: `feature/<domain>-<rand6>` 생성·체크아웃, 푸쉬 시 자동 머지. 이어서 `request.md` 초안 — 기능 설명 / API / "확정 설계 결정사항" Y/N 질문 | `기능 생성` · `/feature-plan` |
| ③ | **사람** | request.md 보완 + 결정사항 답변 후 work 작성 지시 | — |
| ④ | Claude | `work.md` 작성 → 저장 즉시 validator 자동 실행 | (이어짐) |
| ⑤ | **사람** | work.md / change.md 검토 (사전 구현 항목 있으면 먼저 처리) | `작업 시작` · `/feature-implement` |
| ⑥ | Claude | 전제 검증 → 신규/수정 모드 자동 판별 → 구현 코드 + `spec.ts` 생성 (브랜치 결정은 ②에서 끝남) | (이어짐) |
| ⑦ | Claude | spec 실행 → 실패 시 자가수복 (최대 10회) | (이어짐) |
| ⑧ | Claude | 리포트 생성 또는 추기 (신규: `report.md` 신규 / 수정: `## 수정 - <YYMMDD>-<N>` 섹션 append) | (이어짐) |
...
| `기능 수정` · `/feature-modify-plan` | 원본 `work.md` + `report.md` 존재 (이미 구현 완료된 기능만 변경 대상) |
| `작업 시작` · `/feature-implement` | **신규 모드**: `work.md` 존재 + request 결정사항 전부 답변 + 사전 구현 항목 전부 완료(`- [x]`) + validator 통과 <br> **수정 모드**: 미추기 `change-<YYMMDD>-<N>.md` 존재 + 결정사항 답변 + 사전 구현 항목 완료 <br> 인자 없이 호출 시 미구현 플랜을 자동 탐색해 모드 결정 (후보 N개면 사용자에게 선택 요청) |
...
→ **멀티 work 한 사이클 누적**: 같은 자동 브랜치 위에서 `/feature-plan A` → `/feature-plan B` → `/feature-modify-plan C` 식으로 work / change 를 N개 쌓을 수 있음. 두 번째부터는 브랜치 분기 질의가 자동 생략되어 마찰 0. 이후 `/feature-implement` 만 N번 호출하면 (인자 없이) 미구현 후보를 매번 골라 1건씩 구현 — 한 번에 N건 자동 처리는 의도적으로 안 함 (한 사이클 내 검수 지점 보장).
...
모든 항목이 `- [x]` 완료 체크되기 전엔 `작업 시작` (`/feature-implement`) 진입 불가. 본 기능 구현과 인프라 작업이 섞이는 것 방지.
...
| 커스텀 슬래시 커맨드 | `.claude/commands/` | `/harness-init` + 워크플로 6종 (위 [명령 일람](#명령-일람) 표 참조). 한국어 키워드와 동등한 진입점, docs 폴더명과 1:1 매칭 |
...
| `.claude/commands/` 의 플러그인 소유 슬래시 7종 | 제거 (사용자 추가 커맨드는 보존) | 동일 |
