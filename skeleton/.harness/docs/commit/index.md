...
5. **⚠️ 사용자 확인 필수** — 준비된 커밋 메시지와 staged 파일 목록(`git status --short`)을 사용자에게 보여주고 **커밋 진행 여부를 명시적으로 확인**받는다.
   > "아래 내용으로 커밋하겠습니다. 진행할까요?"
   > 메시지 + 파일 목록 출력
   - 사용자가 승인한 경우에만 `git commit` 실행
   - 거부 또는 무응답 상태에서 커밋 금지 (해당 세션에서 앞서 커밋 승인을 받은 이력이 있어도 **매 커밋마다 재확인**)
   - 사용자가 메시지 수정을 요청하면 반영 후 재확인
6. `--no-verify` 금지. husky 훅 실패 시 원인 조사·수정 후 재커밋 (재커밋 시에도 ⑤ 재확인)
